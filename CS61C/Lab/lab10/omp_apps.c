...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    for(int i = thread_id; i < ARRAY_SIZE; i += max_threads)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  {
    int max_threads = omp_get_num_threads();
    int n = ARRAY_SIZE / max_threads;
    int thread_id = omp_get_thread_num();

    int start = thread_id * n;
    int end = (thread_id == max_threads - 1) ? ARRAY_SIZE : start + n;
...
char* compute_dotp(int arr_size) {
  ...
  double fast_reduction_time = 1000.0;
